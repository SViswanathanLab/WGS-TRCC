...
temp_path = "/Users/ananthansadagopan/Documents/ViswanathanLab/tRCC_X/UOK146/ASE_on_hets_merged/"
vals = [f for f in listdir(temp_path) if "chrX_" in f]
KS_pval = []
...
for www in vals:
    ...
    sd_cutoff = 1
    print("reading VCF")
    path1 = "/Users/ananthansadagopan/Documents/ViswanathanLab/tRCC_X/UOK146/ASE_on_hets_raw/uok146_hg38_sorted_picard_filtered.vcf.gz"
    ...
    df = df[df['POS']!=41148687] #Filter USP9X intron with H3K27Ac SNV
    ...
    df = df[df['contig']=="chrX"]

    #Filter to sites with at least 10 RNA reads
    df = df[df['totalCount']>10]

    #Implement DNA read depth + germline VAF standard deviation filters, Note: VAF filter OFF
    ...
    df.to_csv("/Users/ananthansadagopan/Documents/ViswanathanLab/tRCC_X/UOK146/ASE_plots/" + www + "_AF_vs_position.csv", index=False)
    ...
    if "CJ-5681" in value and "Normal" not in value:
        ...
        sub_cn_df = sub_cn_df[sub_cn_df['Chromosome']=="X"]
    ...
    df_cnnew = pd.read_csv("/Users/ananthansadagopan/Documents/ViswanathanLab/tRCC_X/UOK146/A2_CKDN230014513_dnacopy.seg", sep="\t")
    df_cnnew = df_cnnew[df_cnnew['chrom']==23]
    start_vals = df_cnnew['loc.start'].tolist()
    end_vals = df_cnnew['loc.end'].tolist()
    cn_seg = df_cnnew['C'].tolist()
    ...
    if "H3K27Ac" in www:
        axs[1,1].set_ylabel("H3K27Ac Minor Allele Fraction", labelpad=5)
    elif "RNA" in www:
        axs[1,1].set_ylabel("RNA Minor Allele Fraction", labelpad=5)
    elif "MeDIP" in www:
        axs[1,1].set_ylabel("5mC Minor Allele Fraction", labelpad=5)
...
#/Users/ananthansadagopan/Documents/ViswanathanLab/tRCC_X/sTFE/ASE_on_hets_merged/ASE_plots
    ...
    fig.savefig("/Users/ananthansadagopan/Documents/ViswanathanLab/tRCC_X/UOK146/ASE_plots/" + www + "_AF_vs_position.pdf", dpi=dpi_set, bbox_inches = 'tight')

df_out = pd.DataFrame([vals, KS_pval]).T
df_out.columns = ['Sample', 'pval_KS']
df_out.to_csv("/Users/ananthansadagopan/Documents/ViswanathanLab/tRCC_X/UOK146/ASE_plots/unpaired_RNAseq_TCGA_XR_summary_all_samples_KS_test_temp.csv")
