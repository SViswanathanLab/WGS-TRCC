...
for temp_value in values_to_iterate:
    ...
    fig, (ax1, ax2, ax3, ax4, ax5, ax6) = plt.subplots(6,1, gridspec_kw={'height_ratios': [1, 1, 1, 1, 1, 1]})
    ...
    while qx<len(start_vals):
        if float(alleleA_CN[qx]) == float(alleleB_CN[qx]):
            ax6.plot((float(start_vals[qx]), float(end_vals[qx])), (float(alleleA_CN[qx]), float(alleleA_CN[qx])), marker=None, linestyle="-", lw=5, c="gainsboro", alpha=1)
            ax6.plot((float(start_vals[qx]), float(end_vals[qx])), (float(alleleB_CN[qx]), float(alleleB_CN[qx])), marker=None, linestyle="-", lw=5, c="gainsboro", alpha=0.6)
            copyneutral_starts.append(float(start_vals[qx]))
            copyneutral_ends.append(float(end_vals[qx]))
        else:
            ax6.plot((float(start_vals[qx]), float(end_vals[qx])), (float(alleleA_CN[qx]), float(alleleA_CN[qx])), marker=None, linestyle="-", lw=5, c="black", alpha=1)
            ax6.plot((float(start_vals[qx]), float(end_vals[qx])), (float(alleleB_CN[qx]), float(alleleB_CN[qx])), marker=None, linestyle="-", lw=5, c="orange", alpha=0.6)
        qx=qx+1

    valid_pos = []
    all_positions = df2['Adj_Start'].tolist()
    w=0
    while w<len(copyneutral_starts):
        for z in all_positions:
            if z >= int(copyneutral_starts[w]) and z <= int(copyneutral_ends[w]):
    ...
    df2_cn = df2[df2['Adj_Start'].isin(valid_pos)]
    new_pos = df2_cn['Adj_Start'].tolist()
    ys3_cn = np.maximum(df2_cn['AllelicRatio'], (1-df2_cn['AllelicRatio'])).tolist()
    ys4_cn = np.minimum(df2_cn['AllelicRatio'], (1-df2_cn['AllelicRatio'])).tolist()

    df2_inverse = df2[~(df2['Adj_Start'].isin(valid_pos))]
    org_pos = df2_inverse['Adj_Start'].tolist()
    org_pos_cn = np.minimum(df2_inverse['AllelicRatio'], (1-df2_inverse['AllelicRatio'])).tolist()

    #ax4.scatter(xs3,ys3, c="black", alpha=1, s=2)
    ax4.scatter(org_pos,org_pos_cn, c="orange", s=2, alpha=1)
    #ax4.scatter(new_pos,ys3_cn, c="gainsboro", alpha=1, s=2.01)
    ax4.scatter(new_pos,ys4_cn, c="gainsboro", s=2, alpha=1)
    ax5.scatter(xs3,ys6, c="gainsboro", s=2, alpha=1)
    ...
    ax6.grid(False)
    ax6.tick_params(axis='x', which='major', bottom=True, labelsize=15, size=4)
    ax6.tick_params(axis='y', which='major', left=True, labelsize=15, size=4)
    ...
    ax6.set_xlabel("hg38 chrX Coordinate (bp)", fontsize=15)
    ...
    ax6.axvline(49028726, ls="--", lw=2, c="black")

    fig.tight_layout()
    plt.subplots_adjust(hspace=1.2)
    dpi_set = 300
    fig.savefig("/Users/ananthansadagopan/Documents/ViswanathanLab/tRCC_X/CNIs/REVISED_" + temp_value + ".pdf", dpi=dpi_set)
    fig.savefig("/Users/ananthansadagopan/Documents/ViswanathanLab/tRCC_X/CNIs/REVISED_" + temp_value + ".png", dpi=dpi_set)
