...
values_to_iterate = [x for x in values if "titan.ichor.cna.txt" in x and ".pdf" not in x and "png" not in x]
...
for temp_value in values_to_iterate:
    ...
    ys3 = df2['logR_Copy_Number'].tolist()
    ...
    while qx<len(start_vals):
        ax1.plot((float(start_vals[qx]), float(end_vals[qx])), (float(alleleA_CN[qx]), float(alleleA_CN[qx])), marker=None, linestyle="-", lw=3, c="black", alpha=1)
        ax1.plot((float(start_vals[qx]), float(end_vals[qx])), (float(alleleB_CN[qx]), float(alleleB_CN[qx])), marker=None, linestyle="-", lw=3, c="orange", alpha=0.6)
        qx=qx+1

    #ax1.scatter(xs1,[round(x) for x in ys1], c="black", alpha=1, s=2)
    #ax1.scatter(xs2,[round(x) for x in ys2], c="orange", alpha=1, s=2)
    ...
    qx=0
    while qx<len(start_vals):
        ax2.plot((float(start_vals[qx]), float(end_vals[qx])), (float(alleleA_CN[qx]), float(alleleA_CN[qx])), marker=None, linestyle="-", lw=3, c="black", alpha=1)
        qx=qx+1

    #ax2.scatter(xs3,[round(x) for x in ys3], c="black", alpha=1, s=2)
    ...
    ax1.set_ylim([-0.2, 6.1])
    ax2.set_ylim([-0.2, 6.1])
    ...
    ax2.set_ylabel("Copy Number", fontsize=15)
