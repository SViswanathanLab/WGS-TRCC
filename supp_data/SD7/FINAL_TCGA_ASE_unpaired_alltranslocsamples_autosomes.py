...
vals = [f for f in listdir(temp_path) if "chrX_TopHat2" not in f and "TCGA-A8-A09E" not in f]
KS_pval = []
distal_vals_total = []
prox_vals_total = []

lengths = [248956422, 242193529, 198295559, 190214555, 181538259, 170805979, 159345973, 145138636, 138394717, 133797422, 135086622, 133275309, 114364328, 107043718, 101991189, 90338345, 83257441, 80373285, 58617616, 64444167, 46709983, 50818468, 156040895]
main_chr_list = ["chr1", "chr2", "chr3", "chr4", "chr5", "chr6", "chr7", "chr8", "chr9", "chr10", "chr11", "chr12", "chr13", "chr14", "chr15", "chr16", "chr17", "chr18", "chr19", "chr20", "chr21", "chr22", "chrX"]
chr_length_dict = dict(zip(main_chr_list, lengths))

#Iterate over each file
for www in vals:
    value = www.split("_TopHat2_RNA_unpaired_on_germline_hets_unphased_")[1].split(".log")[0]
    try:
    ...
    bp1_part = [x.split("--")[0] for x in samples_df['Fusion'].tolist()]

    bp2LOC_dict = dict(zip(samp1, bp2))
    bp1LOC_dict = dict(zip(samp1, bp1))
    bp1CHR_dict = dict(zip(samp1, bp1_chr))
    bp1PARTNER_dict = dict(zip(samp1, bp1_part))

    temp_chr = bp1CHR_dict[temp_id]
    temp_loc = int(bp1LOC_dict[temp_id])
    temp_partner = bp1PARTNER_dict[temp_id]
    temp_length = int(chr_length_dict[temp_chr])
    ...
    df.to_csv("/Users/ananthansadagopan/Documents/ViswanathanLab/tRCC_X/TCGA/TCGA_chrX_translocations_plots/" + www + "_AF_vs_position_autosomes.csv", index=False)
    ...
    ideodf = pd.read_csv("/Users/ananthansadagopan/Documents/ViswanathanLab/tRCC_X/cytoBandIdeo.txt", sep="\t", header=None)
    ideodf = ideodf[ideodf[0]==temp_chr]
    val1_ideo = ideodf[1].tolist()[0]
    val2_ideo = ideodf[2].tolist()[0]
    ideodf.to_csv("/Users/ananthansadagopan/Documents/ViswanathanLab/tRCC_X/cytoBandIdeo_" + temp_chr + ".txt", sep="\t", header=None, index=False)
    ...
    def ideograms(fn):
        ...
        xranges, colors = [(val1_ideo,val2_ideo)], [(.7, .7, .7)]
        ymin = 0
    ...
    fig.savefig("/Users/ananthansadagopan/Documents/ViswanathanLab/tRCC_X/TCGA/TCGA_chrX_translocations_plots/fusionPARTNER_" + www + "_AF_vs_position.pdf", dpi=dpi_set, bbox_inches = 'tight')
...
df_out.to_csv("/Users/ananthansadagopan/Documents/ViswanathanLab/tRCC_X/TCGA/TCGAchrXtransloc_unpaired_RNAseq_TCGA_XR_summary_all_samples_KS_test_temp_autosomes.csv")
