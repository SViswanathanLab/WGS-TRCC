...
#Compiled seg file
df_abs_cn = pd.read_csv("WGS_solutions.csv")

#Defining escapee exon boundaries (creating list of excluded regions)
chrX_exons_df = pd.read_csv("chrX_all_exons_gencode_v43_UCSC.csv")
gene_classes_df = pd.read_excel("kidney_specific_chrX_gene_class_list_cotton.xlsx")
...
temp_path = "ASEReadCounter_output/"
vals = [f for f in listdir(temp_path) if "chrX_RNA_paired_on_germline_hets_unphased" in f]
KS_pval = []
...
for www in vals:
    ...
    print("reading VCF")
    all_files_in_dir = os.listdir("germline_hets/")
    temp_DNA_id = [x for x in all_files_in_dir if temp_id in x]
    path1 = "germline_hets/" + temp_DNA_id[0]
    ...
    for xranges, yrange, colors, label in ideograms('cytoBandIdeo_chrX.txt'):
        print(xranges)
    ...
    fig.savefig(str(www) + "_AF_vs_position.pdf", dpi=dpi_set, bbox_inches = 'tight')

df_out = pd.DataFrame([vals, KS_pval]).T
df_out.columns = ['Sample', 'pval_KS']
df_out.to_csv("pvalues_summary.csv")