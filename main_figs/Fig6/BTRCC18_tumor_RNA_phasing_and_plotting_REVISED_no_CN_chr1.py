...
path2 = "/Users/ananthansadagopan/Documents/ViswanathanLab/tRCC_X/WGS/Xi_Xa_fusion/BTRCC18_phased/chr1_het_eagle2phased.vcf.gz"
...
#tumors_with_Xq_del = ['LungPlTum_1', 'LungPlTum_3']
...
for www in vals:
    ...
    fig, axs = plt.subplots(7, 3, sharey='row', gridspec_kw={'height_ratios': [4,4,9,9,6,1,1], 'width_ratios': [1, 3, 1]})
    d = {}
    yticks = []
    yticklabels = []
    ...
    while qx<len(start_vals):
        ...
        if tempval == 0:
            ...
            #pass
            axs[0,1].plot((float(start_vals[qx]), float(end_vals[qx])), (float(new_temp1[qx]), float(new_temp1[qx])), marker=None, linestyle="-", lw=3, c="green", zorder=10)
            axs[0,1].plot((float(start_vals[qx]), float(end_vals[qx])), (float(new_temp2[qx]), float(new_temp2[qx])), marker=None, linestyle="-", lw=3, c="green", zorder=10)  
    ...
    while qx<len(start_vals):
        #pass
        if start_vals[qx]<35176378:
            axs[1,1].plot((float(start_vals[qx]), float(end_vals[qx])), (float(new_temp1[qx]), float(new_temp1[qx])), marker=None, linestyle="-", lw=3, c="orange")
            axs[1,1].plot((float(start_vals[qx]), float(end_vals[qx])), (float(new_temp2[qx]), float(new_temp2[qx])), marker=None, linestyle="-", lw=3, c="black", alpha=0.6)
        else:
    ...
    df.to_csv("/Users/ananthansadagopan/Documents/ViswanathanLab/tRCC_X/WGS/Xi_Xa_fusion/BTRCC18_phased/chr1_unphased_DNA_and_RNA.csv", index=False)
    ...
    axs[2,1].scatter(xs,ys, c=clist, alpha=1, s=6.5)
    axs[2,1].scatter(xs,[1-y for y in ys], c=clist_inv, alpha=1, s=6.5)
    ...
    axs[3,1].scatter(xs,new_ys_vals, c=clist, alpha=1, s=6.5)
    axs[3,1].scatter(xs,[1-y for y in new_ys_vals], c=clist_inv, alpha=1, s=6.5)
    ...
    pdf = count / sum(count)
    cdf = np.cumsum(pdf)
    axs[5,1].plot(bins_count[1:], cdf, c="black")
    axs[5,1].set_xlim([-1000000, hg38_chr_length+1000000])
    axs[5,1].set_ylim([-0.01, 1])
    ...
    axs[5,1].plot((max(xs), hg38_chr_length), (1, 1), linewidth=1.4, c="black", marker=None, dash_capstyle="butt")
    axs[5,1].plot((0, x_min-500000), (0, cdf_min), linewidth=1.4, c="black", marker=None, dash_capstyle="butt")
    ...
    pdf = count / sum(count)
    cdf = np.cumsum(pdf)
    axs[5,1].plot(bins_count[1:], cdf, c="red")
    axs[5,1].set_xlim([-1000000, hg38_chr_length+1000000])
    ...
    x_min = bins_count[np.argmax(cdf <= 0)]
    cdf_min = np.min(cdf)
    axs[5,1].plot((x_max, hg38_chr_length), (1, 1), linewidth=1.4, c="red", marker=None, dash_capstyle="butt")
    axs[5,1].plot((0, x_min-500000), (0, cdf_min), linewidth=1.4, c="red", marker=None, dash_capstyle="butt")

    patch1 = mpatches.Patch(color="red", label="Biallelic\nSNVs (RNA)")
    patch2 = mpatches.Patch(color="black", label="Expressed\nSNVs")

    legend = axs[5,1].legend(handles=[patch1, patch2], bbox_to_anchor=(-0.25, 0.84))
    ...
    #legend = axs[4,1].legend(handles=[patch1, patch2], bbox_to_anchor=(-0.25, 0.64), title="Haplotype")
    ...
    #legend = axs[3,1].legend(handles=[patch1, patch2], bbox_to_anchor=(-0.25, 1), title="Copy Number")

    #frame = legend.get_frame()
    #frame.set_facecolor('white')
    #frame.set_edgecolor('black')

    axs[5,1].set_facecolor("white")
    axs[5,1].grid(False)
    axs[5,1].tick_params(axis='x', which='major', bottom=True, labelsize=10, size=4) 
    axs[5,1].tick_params(axis='y', which='major', left=True, labelleft=True, labelsize=10, size=4) 
    axs[5,1].spines['bottom'].set_color('0')
    axs[5,1].spines['left'].set_color('0')
    ...
    #axs[4,1].axvspan(0, 49028726, alpha=0.15, color='red')
    #axs[4,1].axvspan(49043410, 156040895, alpha=0.15, color='blue')

    #Label axes
    axs[5,1].set_ylabel("Cumulative Probability")
    axs[5,1].set_xlabel("hg38 " + temp_chr + " Position (bp)", labelpad=5)
    axs[2,1].set_ylabel("Allele Fraction (RNA)", labelpad=5)
    axs[2,1].set_xlabel("hg38 " + temp_chr + " Position (bp)", labelpad=5)
    axs[3,1].set_ylabel("Allele Fraction (DNA)", labelpad=5)
    ...
    axs[0,1].set_yticklabels([0, 1])
    axs[1,1].set_ylim([0, 2.5])
    axs[0,1].axvline(35176378, c="black", lw=0.5, ls="--")
    axs[1,1].axvline(35176378, c="black", lw=0.5, ls="--")
    ...
    axs[5,1].axvline(35176378, c="black", lw=0.5, ls="--")
    ...
    axs[4,1].tick_params(axis='y', which='major', left=True, labelsize=10, size=4, labelleft=True)
    #axs[4,1].axhline(statistics.median(maf_vals_finalized), c="blue", lw=0.5, ls="--")

    fig.savefig("/Users/ananthansadagopan/Documents/ViswanathanLab/tRCC_X/WGS/Xi_Xa_fusion/BTRCC18_phased/" + www + "_AF_vs_position_" + temp_chr + "_no_CN.pdf", dpi=dpi_set, bbox_inches = 'tight')
    qqx=qqx+1
